...
//			@RequestParam(name = "branchId", defaultValue = "1") int branchId,
//			@RequestParam(name = "sortBy", defaultValue = "themaId") String sortBy,
//			@RequestParam(value = "date", required = false) LocalDate date, HttpSession session, String themaName,
...
//	        themaList = themaRepository.findAllByTitleContaining(themaName, pageable);
//	    } else {
...
//	}
	...
	public String addWishList(int themaId, HttpSession session, Model model) {
		...
		return "redirect:/thema/intro"; // 사용자를 이전 페이지로 리디렉션하거나 다른 페이지로 이동
	}
	...
	// ================== 테마 관련 ===================//
		...
		@PostMapping("/reg")
		public String submitThema(@ModelAttribute Thema thema, Model model, HttpServletRequest request, String[] time,
				int branchId) {
			...
			public String submitThemaDelete(@PathVariable("themaId") int themaId, HttpServletRequest request,  PageData pageData) {
