...
public class SuperController {
	...
	public String supermain(Model model, HttpSession session) {
		...
		List<Object[]> themaTotal = reservationRepository.thema_price();
		List<Object[]> branchTotal = reservationRepository.getBranch_total();
	...
	public String loginSuperUser(String superId, String superPw, Model model, HttpSession session) {
		...
		if (superService.loginSuperUser(superId, superPw)) {
			...
			return "/super/super_login"; // 로그인 실패 시 다시 로그인 페이지로
		}
	}
	...
	@GetMapping("/rvList")
	String rvlist(String userName,
			@PageableDefault(page = 0, size = 15, sort = "rvId", direction = Direction.ASC) Pageable pageable,
		...
		Page<Reservation_backup> blackList =reservationBackUpRepository.findAll(pageable);
	...
	public String getAll(Model model, HttpSession session, @RequestParam(required = false) String userName,
		...
		if (branchName.equals("전체") || branchName == null) {
			...
			if (userName == null || userName.isEmpty() || userName.equals("null")) {
				...
				blackList = reservationBackUpRepository.findByBranchNameAndUserNameContaining(branchName, userName, pageable);
	...
	public String processAdminSignupForm(@Valid @ModelAttribute("admin") Admin admin, BindingResult br,
			@RequestParam("adminPw1") String adminPw1) {
		if (br.hasErrors()) {
	...
	public String submitBranchUpdate(@ModelAttribute Branch branch, Model model, HttpSession session) {
		...
		if(session.getAttribute("userRole")==null) {
			int a = (int) session.getAttribute("adminBranchId");
			return "redirect:/admin/branch/update/"+a ;
	...
	public String showBranchDeleteForm(@PathVariable("branchId") int branchId,
			Model model, HttpSession session,PageData pageData) {
	...
	@GetMapping("/adminModify/{adminId}")
	   public String showAdminModifyForm(@PathVariable String adminId, Model model) {
	   ...
	   public String showAdminModifyReg(
	           PageData pageData,
	           @Valid @ModelAttribute("admin") Admin updateAdmin,
	           BindingResult br,
